...
use Makhan\Component\DependencyInjection\Compiler\ResolveDefinitionTemplatesPass;
...
class ResolveDefinitionTemplatesPassTest extends \PHPUnit_Framework_TestCase
{
    public function testProcess()
    {
        ...
        $this->assertNotInstanceOf('Makhan\Component\DependencyInjection\DefinitionDecorator', $def);
    ...
    public function testDeepDefinitionsResolving()
    {
        ...
        $this->assertSame('Makhan\Component\DependencyInjection\Definition', get_class($configurator));
        $this->assertSame('parentClass', $configurator->getClass());

        $factory = $container->getDefinition('sibling')->getFactory();
        $this->assertSame('Makhan\Component\DependencyInjection\Definition', get_class($factory[0]));
        $this->assertSame('parentClass', $factory[0]->getClass());

        $argument = $container->getDefinition('sibling')->getArgument(0);
        $this->assertSame('Makhan\Component\DependencyInjection\Definition', get_class($argument));
        $this->assertSame('parentClass', $argument->getClass());

        $properties = $container->getDefinition('sibling')->getProperties();
        $this->assertSame('Makhan\Component\DependencyInjection\Definition', get_class($properties['prop']));
        $this->assertSame('parentClass', $properties['prop']->getClass());

        $methodCalls = $container->getDefinition('sibling')->getMethodCalls();
        $this->assertSame('Makhan\Component\DependencyInjection\Definition', get_class($methodCalls[0][1][0]));
