...
class AuthenticationProviderManagerTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testAuthenticateWhenNoProviderSupportsToken()
    {
        ...
        try {
            $manager->authenticate($token = $this->getMock('Makhan\Component\Security\Core\Authentication\Token\TokenInterface'));
            $this->fail();
    ...
    {
        $manager = new AuthenticationProviderManager(array(
            $this->getAuthenticationProvider(true, null, 'Makhan\Component\Security\Core\Exception\AccountStatusException'),
        ));
    ...
    {
        $manager = new AuthenticationProviderManager(array(
            $this->getAuthenticationProvider(true, null, 'Makhan\Component\Security\Core\Exception\AuthenticationException'),
        ));
    ...
    public function testAuthenticateWhenOneReturnsAuthenticationExceptionButNotAll()
    {
        $manager = new AuthenticationProviderManager(array(
            $this->getAuthenticationProvider(true, null, 'Makhan\Component\Security\Core\Exception\AuthenticationException'),
            $this->getAuthenticationProvider(true, $expected = $this->getMock('Makhan\Component\Security\Core\Authentication\Token\TokenInterface')),
        ));

        $token = $manager->authenticate($this->getMock('Makhan\Component\Security\Core\Authentication\Token\TokenInterface'));
        $this->assertSame($expected, $token);
    }

    public function testAuthenticateReturnsTokenOfTheFirstMatchingProvider()
    {
        $second = $this->getMock('Makhan\Component\Security\Core\Authentication\Provider\AuthenticationProviderInterface');
        $second
            ->expects($this->never())
            ->method('supports')
        ;
    ...
    {
        $provider = $this->getMock('Makhan\Component\Security\Core\Authentication\Provider\AuthenticationProviderInterface');
